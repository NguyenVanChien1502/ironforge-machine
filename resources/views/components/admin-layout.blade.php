...
<head>
    ...
    <title>{{ $title }} — IronForge Admin</title>
    ...
    <div class="flex min-h-screen">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="flex items-center gap-2 border-b border-white/10 px-6 py-5">
                ...
                <span class="text-lg font-bold">IronForge<span class="text-gold">.</span></span>
            </div>
            ...
            <div class="absolute bottom-0 w-full border-t border-white/10 p-4">
                <a href="{{ route('home') }}" target="_blank" class="mb-2 block text-xs text-gray-400 hover:text-gold">View Site &rarr;</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full rounded-md bg-white/10 px-4 py-2 text-sm font-medium text-white hover:bg-white/20">Logout</button>
                </form>
            </div>
        </aside>
