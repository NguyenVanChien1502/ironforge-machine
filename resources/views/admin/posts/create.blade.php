    ...
    <div class="max-w-3xl rounded-xl bg-white p-8 shadow-sm ring-1 ring-gray-100">
        <form method="POST" action="{{ route('admin.posts.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" required value="{{ old('title') }}" class="input" placeholder="Enter article title">
                @error('title') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="label">Slug</label>
                <input type="text" name="slug" required value="{{ old('slug') }}" class="input" placeholder="article-slug">
                @error('slug') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="label">Excerpt (Brief Summary)</label>
                <textarea name="excerpt" rows="2" class="input" placeholder="Write a short summary for search results and previews">{{ old('excerpt') }}</textarea>
                @error('excerpt') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="label">Body Content</label>
                <textarea name="body" rows="10" required class="input" placeholder="Write the full content of the article here...">{{ old('body') }}</textarea>
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-400">Supported formats: JPG, PNG, WEBP. Max size: 4MB.</p>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="flex items-center pt-8">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_published" value="1" @checked(old('is_published')) class="rounded border-gray-300 text-gold focus:ring-gold h-4 w-4">
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-100">
                <button type="submit" class="btn-secondary">Save Article</button>
