<x-admin-layout title="Thêm bài viết">
    <div class="max-w-3xl rounded-xl bg-white p-8 shadow-sm ring-1 ring-gray-100">
        <form method="POST" action="{{ route('admin.posts.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" required value="{{ old('title') }}" class="input" placeholder="Nhập tiêu đề bài viết">
                @error('title') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="label">Đường dẫn</label>
                <input type="text" name="slug" required value="{{ old('slug') }}" class="input" placeholder="duong-dan-bai-viet">
                @error('slug') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="label">Tóm tắt ngắn</label>
                <textarea name="excerpt" rows="2" class="input" placeholder="Nhập mô tả ngắn để hiển thị ở danh sách và SEO">{{ old('excerpt') }}</textarea>
                @error('excerpt') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="label">Nội dung bài viết</label>
                <textarea name="body" rows="10" required class="input" placeholder="Nhập nội dung đầy đủ của bài viết...">{{ old('body') }}</textarea>
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-400">Hỗ trợ JPG, PNG, WEBP. Tối đa 4MB.</p>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="flex items-center pt-8">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_published" value="1" @checked(old('is_published')) class="h-4 w-4 rounded border-gray-300 text-gold focus:ring-gold">
            ...
            <div class="flex gap-3 border-t border-gray-100 pt-4">
                <button type="submit" class="btn-secondary">Lưu bài viết</button>
