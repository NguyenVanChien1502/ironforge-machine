    ...
    <div class="max-w-xl rounded-xl bg-white p-8 shadow-sm ring-1 ring-gray-100">
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-5">
            ...
            <div class="flex gap-3">
                <button type="submit" class="btn-secondary">Update Category</button>
                <a href="{{ route('admin.categories.index') }}" class="btn-outline !text-charcoal !border-gray-300">Cancel</a>
            </div>
        </form>
    </div>
</x-admin-layout>
