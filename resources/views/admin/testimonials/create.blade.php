    ...
    <div class="max-w-2xl rounded-xl bg-white p-8 shadow-sm ring-1 ring-gray-100">
        <form method="POST" action="{{ route('admin.testimonials.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="customer_name" required value="{{ old('customer_name') }}" class="input" placeholder="e.g. John Doe">
                @error('customer_name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="label">Company / Designation</label>
                <input type="text" name="company" value="{{ old('company') }}" class="input" placeholder="e.g. Acme Construction Group">
            ...
            <div>
                ...
                <select name="rating" class="input">
                    @for($i = 5; $i >= 1; $i--)
                        <option value="{{ $i }}" @selected(old('rating', 5) == $i)>{{ $i }} Stars</option>
                    @endfor
                </select>
            ...
            <div>
                ...
                <textarea name="content" rows="4" required class="input" placeholder="Write customer feedback here...">{{ old('content') }}</textarea>
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-400">Supported formats: JPG, PNG, WEBP. Max size: 2MB. Leave empty to use initials avatar.</p>
                @error('avatar') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center">
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_visible" value="1" @checked(old('is_visible', true)) class="rounded border-gray-300 text-gold focus:ring-gold h-4 w-4">
                    <span class="ml-2 text-sm font-medium text-gray-700">Display publicly on website</span>
                </label>
            </div>

            <div class="flex gap-3 pt-4 border-t border-gray-100">
                <button type="submit" class="btn-secondary">Save Testimonial</button>
